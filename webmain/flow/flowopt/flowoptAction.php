...
class flowoptClassAction extends Action
{
	...
	public function checkAjax()
	{
		...
		$sm 	= $this->post('sm');
		$xgwj 	= $this->post('xgwj');
		$rgfeelist 	= $this->post('rgfeelist');
		$clupdatelist 	= $this->post('clupdatelist');
		$totalprice 	= $this->post('totalprice');
		$alltotal 	= $this->post('alltotal');
		$msg 	= m('flow')->opt('check', $modenum, $mid, $zt,$sm,$xgwj,$rgfeelist,$totalprice,$alltotal,$clupdatelist);
		backmsg('', $msg);
	}
