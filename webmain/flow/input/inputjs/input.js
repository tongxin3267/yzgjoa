var ismobile=0,firstrs={},alldata={};
function initbodys(){};
...
var c={
	...
	savesss:function(){
		...
		if(firstrs.checktype=='change' && mid==0){
			if(!d.sysnextoptid && form('sysnextopt')){
				this.showtx('请选择['+firstrs.name+']处理人');
	...
	//初始上传框
	initinput:function(){
		var o,o1,sna,i,tsye,uptp,farr=alldata.filearr,far;
		var o = $('div[id^="filed_"]');
		if(isedit==1)o.show();
		for(i=0;i<o.length;i++){
			o1 = o[i];sna= $(o1).attr('tnam');tsye=$(o1).attr('tsye');
			if(isedit==1){
				uptp = 'image';
				if(tsye=='file')uptp='*';
				$.rockupload({
