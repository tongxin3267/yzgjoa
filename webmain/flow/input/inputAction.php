...
class inputAction extends ActionNot
{
	...
	public function saveAjax()
	{
		...
		if(in_array('routeline', $allfields)){
			
			$routeline=$this->post('routeline');
			$routelinetext=$this->post('routelinetext');
			if (!empty($routelinetext)&&strpos($routeline,$routelinetext)!== false) {
				//echo "string";
			}else{
				$uaarr['routeline'] = $routelinetext.$routeline;		
				/*if ($this->post('weizhi')) {
					if (!empty($routelinetext)) {
						# code...
						$uaarr['routeline'] = $routelinetext.'-'.$routeline;		

					}

					$weizhi=$this->post('weizhi');
					if (!empty($weizhi)&&strpos($weizhi,$routeline)!== false) {
						//echo "string";
					}else{
						$uaarr['weizhi'] = $uaarr['routeline'].$weizhi;
					}	
				}*/					
				//echo "string2".$routelinetext.$routeline;
			}
		}
		...
		//var_dump($allfields);
		if(in_array('bookid', $allfields)||in_array('rzgongdiid', $allfields)||in_array('jzgongdiid', $allfields)){	
			$bookid=$uaarr['bookid'];		
			$rzgongdiid=$uaarr['rzgongdiid'];	
			$jzgongdiid=$uaarr['jzgongdiid'];	
			if($bookid>0||$rzgongdiid>0||$jzgongdiid>0){

				if(!isempt($bookid)) {
					$row = m('book')->getone("`id`='$bookid'",'*');
				}
				if(!isempt($rzgongdiid)) {
					$row = m('rzgongdi')->getone("`id`='$rzgongdiid'",'*');				
				}
	...
	private function luactions($slx=0, $stwhe='')
	{
		...
		if($mid==0){
			...
			//if(!$isadd) echo "<script> alert('无权添加[".$moders['name']."]的数据;');history.go(-1); </script>";
			
			if(!$isadd) echo "<script> alert('无权添加[".$moders['name']."]的数据;');history.go(-1);  window.close();</script>";
		}
		...
		foreach($list[1] as $k=>$nrs){
			$str		= $this->inputobj->getfieldcont($nrs, $this->actclss, $leox='', $iszb=0,$this->rs);
