...
class Action extends mainAction
{
	...
	public function publicstoreAjax()
	{
		...
		$rows=$this->getyrecorddata($rows,$modenum);
		// $this->returnjson($rows);

		//安装测量 计算总价 happy 20181216 begin
		if ($table=='buildin') {
			$res 		= m('buildin a')->getone($where,'SUM(totalprice) totalprice,SUM(alltotal) alltotal');

			$bacarr	= array(
				'totalCount'=> $total,
				'totalprice'=> $res['totalprice'],
				'alltotal'=> $res['alltotal'],
				'rows'		=> $rows
			);
		//安装测量 计算总价 happy 20181216 end			
		}elseif ($table=='clpaifa') {
			$res 		= m('clpaifa')->getone($where,'SUM(totalprice) totalprice,SUM(alltotal) alltotal');
		...
		}elseif ($table=='book'||$table=='rzgongdi'||$table=='yzjuzhuang') {
			$res 		= m($table)->getone($where,'SUM(price) totalprice');
			$bacarr	= array(
				'totalCount'=> $total,
				'totalprice'=> $res['totalprice'],
				'rows'		=> $rows
			);
		//安装测量 计算总价 happy 20190612 end			
		}else{
			$bacarr	= array(
				'totalCount'=> $total,
				'rows'		=> $rows
	...
	*/
	...
	{/**/
		//happy_add   导出更多数据
		if ($this->post('modenum')=='customer') {
			$fields = 'laiyuan,mobile,routeline,name,tel,address,unitname,linkname,zxdate,budgettype,zxstyle,fpdate,gddesigner,rzdesigner,status,rzstatus,compet,type,remark';
			$header = '渠道,编号,区域,客户姓名,联系方式,小区名称,面积,户型,装修时间,预算,风格,分配时间,硬装设计师,软装设计师,硬装状态,软装状态,竞争公司,合作公司,客户要求';
			$fields = explode(',', $fields);
			$header = explode(',', $header);
		}else if ($this->post('exceltitle','',1)=='全部工地') {
			
			$fields = 'nbbh,title,chuban,telephone,weizhi,housesize,housetype,budgettype,designer,mdarea,author,price,telwatpri,cbdt,endtime,explain,designerpromise,coursename,status';
			$header = '内部编号,项目名称,业主姓名,联系方式,装修地址,面积,户型,预算类型, 设计师,门店经理,工程监理, 总合同价,含水电价,开始时间,预计结束时间,说明,设计师承诺,进度,状态';
			$fields = explode(',', $fields);
			$header = explode(',', $header);
		}else{
	...
	*	显示z最新跟进记录
	*/
	private function getyrecorddata($rows,$modenum)
	{
		$adb	= m('flow_log');
		foreach($rows as $k=>$rs){
			...
			// $rows[$k]['record'] = '<span class="record-pc" title="' . $record . '"  >' . $record . '</span>';
		}
		return $rows;
	}
}
