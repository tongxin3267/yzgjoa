...
class fileClassModel extends Model
{
	...
	public function getstr($mtype, $mid, $lx=0)
	{
		...
		foreach($filearr as $k=>$rs){
			...
			if($flx=='jpg'||$flx=='png'||$flx=='jpeg'){
				//var_dump($rs);
				$i=m('file')->showimage($rs['id']);
				$nnn=explode(".",$i); 
				$i2=$nnn[0].'_s.'.$nnn[1];
				//文件预览那里

				$fstr.= '<img src="'.$i2.'" align="absmiddle" height=50 width=50 class="showimage"  data-preview-src="'.$i.'" data-preview-group="1" >('.$rs['filesizecn'].''.$str1.')';
