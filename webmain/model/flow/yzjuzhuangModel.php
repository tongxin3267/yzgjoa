<?php
class flow_yzjuzhuangClassModel extends flowModel
{	// happy_add 读取工长来审批
	...
	protected function flowcheckname($num)
	{
		...
		if($num=='over'){
			...
			$mdarea = m('userinfo')->getone('`name`="'.$this->rs['mdarea'].'"');
			return array($xunjian['id'].','.$chuban['id'].','.$chengjia['id'].','.$mdarea['id'],$xunjian['name'].','.$chuban['name'].','.$chengjia['name'].','.$mdarea['name']);
		}
	}
	public function flowrsreplace($rs,$isv=0)
	{
		...
		// 状态格式化
		$rers 		= $this->db->getone('[Q]flow_bill', '`mid`="'.$rs['id'].'" and modeid=59');
		$wdst		 = $rs['status'];
		if($wdst==0&&$rers['status']==0)$status='待<font color="blue">'.$rers['nowcheckname'].'</font>处理';
		$rs['status']=$status;

		// 计算工期
		if($rs['courseid']==137||$rs['courseid']==138||$rs['courseid']==139||$rs['status']==1||$rers['status']==1){
			$dif="已完工";
		}else{			
