<?php
class flow_rzgongdiClassModel extends flowModel
{	// happy_add 读取工长来审批
	public function initModel()
	{
		...
		$this->brandarr		 = c('array')->strtoarray('元贞国际|#888888,贞筑豪宅|#888888,梦依达|#888888,域嘉|#888888');
	...
	protected function flowcheckname($num)
	{
		...
		if($num=='allxg'){
			...
			return array($mdarea['id'].','.$chuban['id'].',1',$mdarea['name'].','.$chuban['name'].',admin');
		}
	}
	public function flowrsreplace($rs,$isv=0)
	{
		...
		// 状态格式化
		$rers 		= $this->db->getone('[Q]flow_bill', '`mid`="'.$rs['id'].'" and modeid=55');
		$wdst		 = $rs['status'];
