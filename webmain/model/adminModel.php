...
class adminClassModel extends Model
{
	...
	*/
	...
	public function getuser($lx=0,$where="`status`=1")
	{
		$rows = $this->getall($where,'id,name,deptid,deptname,deptallname,ranking,tel,face,sex,email,pingyin','sort,name');
		$py   = c('pingyin');
		foreach($rows as $k=>$rs){
	...
	*/
	...
	{
		$where = " and ($qz`name` like '%$key%' or $qz`user` like '%$key%' or $qz`deptallname` like '%$key%' or $qz`ranking` like '%$key%' or $qz`pingyin` like '$key%' or $qz`tel` like '$key%' or $qz`mobile` like '$key%' $ots)";
		return $where;
	}
}