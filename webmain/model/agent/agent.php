	...
	*/
	...
	public function getdatalimit($uid, $lx)
	{
		...
		foreach($rows as $k=>$rs){
			...
			if(isset($rs['footcon']))$jarr['footcon']= $rs['footcon'];	//加显示字段 多展示点数据
		...
		if ($this->agentnum=='buildin') {
			$res 		= m('buildin a')->getone($where,'SUM(totalprice) totalprice,SUM(alltotal) alltotal');

			$arr['totalprice'] 	= $res['totalprice'];
			$arr['alltotal'] 	= $res['alltotal'];
		}elseif ($this->agentnum=='clpaifa') {
