<?php
class admingClassModel extends Model
{
	...
	*/
	...
	/**
	*	根据人员id判断是不是head，如果是就判断组id及组成员，如果不是，返回
	*	$lx 0 全部下级，1直属下级
	*	return 字符串条件
	*/
	/**
	...
	*/
	...
	/**
	*	获取对应部门负责人(15设计部，19市场部)
	*/
	public function getdeptid($uid,$fid)
	{
		$drs 	= $this->db->getone('[Q]dept','headid='.$uid.' ');
		//加了品牌，所以这里的不上级id先取消筛选   happy20180226

		//$drs 	= $this->db->getone('[Q]dept','headid='.$uid.' and (pid=15 or pid=19)');
		if(!$drs)return false;
		$deptid 	= $drs['id'];
		$ids=$this->getdownsbydeptid($deptid,$fid);

		if(isempt($ids))return false;
		return $ids;
	}
	...
	*	根据部门id获取下级人员id
	*/
	public function getdownsbydeptid($deptid,$fid)
	{
		$where = "instr(deptpath,'[$deptid]')>0";
		$rows = $this->getall($where, 'id');
		$s	  = '';
		foreach($rows as $k=>$rs){
			$s.=','.$rs['id'];
		...
		if($s != '')$s = substr($s, 1);
		$where = ''.$fid.' in('.$s.') or markerid in('.$s.')  or rzdesignerid in('.$s.') ';
		...
		foreach($rows as $k=>$rs){
			//$s.=','.$rs['id'];
			$barr[] = ''.$fid.' in('.$rs['id'].') or markerid in('.$rs['id'].') ';
		}
		$where = join(' or ', $barr);
		$where = '('.$where.')';*/
		//if($s != '')$s = substr($s, 1);
	...
	/**
	*	获取是不是对应部门负责人(15设计部，19市场部)
	*/
	public function getisdeptid($uid)
	{
		$drs 	= $this->db->getone('[Q]dept','headid='.$uid.' ');
		//加了品牌，所以这里的不上级id先取消筛选   happy20180226
	...
	public function getdeptids($uid)
	{
		$drs 	= $this->db->getone('[Q]dept','headid='.$uid.' ');
		...
		if(!$drs)return false;
		$deptid 	= $drs['id'];
		$ids=$this->getdownsbydeptids($deptid);
	...
	*	根据部门id获取下级人员ids
	*/
	public function getdownsbydeptids($deptid)
	{
		$where = "instr(deptpath,'[$deptid]')>0";
		$rows = $this->getall($where, 'id');
		$s	  = '';
