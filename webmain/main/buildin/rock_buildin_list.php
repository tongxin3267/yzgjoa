...
$(document).ready(function(){
	...
	totalshow=false;//oa工地统计，只显示管理员，财务部，门店，设计部
	if (deptname.indexOf("管理层")>=0||deptname.indexOf("财务部")>=0||deptname.indexOf("门店")>=0||deptname.indexOf("设计部")>=0) {
		totalshow=true;
	}
	if ((atype=='anzdcl'||atype=='anzhis'||atype=='tuihuohis') && totalshow) {$('#hahah_{rand}').show();}
	...
	var c = {
		...
		clickwin:function(o1,lx){
			...
			var url = 'task.php?a=clpf&num=buildin&mid='+id+'&show=pc&type=0';
			// var url = 'index.php?a=history&m=buildin&d=main&id='+id;
			//js.location(url);
			
			var s='',tit=name;if(!tit)tit='安装测量详情';
			...
			return false;
			//addtabs({num:'admin'+id+'',url:'main,buildin,edit,id='+id+'',icons:icon,name:name+'安装测量'});
		},
		piliang:function(){
			adminusermanage = a;
...
<td width="220" style="display:none">
	...
	<td width="2%"></td>
	<td width="40%" id="hahah_{rand}" style="display: none;"> 合计：<em id="all_{rand}" ></em> &nbsp;&nbsp;&nbsp;&nbsp;商定总价：<em id="total_{rand}" ></em></td>
	...
	<td align="right" nowrap>
		...
		<button class="btn btn-info" id="edit_{rand}" click="clickwin,1" disabled type="button"><i class="icon-edit"></i> 安装测量记录 </button> &nbsp; 
	</td>
</tr>
</table>
</div>
