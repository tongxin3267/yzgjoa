...
class flowClassAction extends Action
{
	...
	public function loadbookdatacourseAjax()
	{
		$setid	= (int)$this->get('setid');
		$where 	= array(
			'setid'		=> $setid
		);
		//$data	= m('flow_course')->getrows($where);
	...
	*/		
	...
	public function loadbookdatacourseAjax()
	{
		...
		//审核步骤
		$data['course']	= m('flow_course')->getall("`setid`='$setid' order by `sort`",'*');
		//渠道获取
		$laiyuan	= m('flow_element')->getone("`mid`='7' and `fields`='laiyuan' order by `sort`",'*');
		$data['laiyuan'] = explode(',', $laiyuan['data']);
		...
		//获取设计师
		//$data['design']	= m('admin')->getall("`deptid` in(15,21,22,25,23,26) order by `sort`",'*');
		$designdetpid = getconfig('designdetpid');
		$data['design']	= m('admin')->getall("`deptid` in($designdetpid) order by `sort`",'*');
		//获取工程监理
		$jianlidetpid = getconfig('jianlidetpid');
		$data['manage']	= m('admin')->getall("`deptid` in($jianlidetpid)  order by `sort`",'*');

		//获取市场顾问		
		$shichangdetpid = getconfig('shichangdetpid');
		$data['shichang']	= m('admin')->getall("`deptid` in($shichangdetpid) order by `sort`",'*');

		echo json_encode($data);
	}

	public function loadbuildindatacourseAjax()
	{
		//获取工程监理
		$data['clgys'] = m('admin')->getall("instr(`deptpath`,'[38]')>0 order by `deptid`",'id,name');

		//获取工程监理
		$data['author'] = m('admin')->getall("instr(`deptpath`,'[8]')>0 order by `deptid`",'id,name');

		echo json_encode($data);
	}

	public function loadclpaifadatacourseAjax()
	{
		//获取工程监理
		$data['clgys'] = m('admin')->getall("instr(`deptpath`,'[35]')>0 order by `deptid`",'id,name');
...
class mode_'.$modenum.'ClassAction extends inputAction{
	...
	*/	
	...
	//应用获取跟进记录
	public function getyrecorddataAjax()
	{
