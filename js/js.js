...
js.rgfeezizuo=function(call,can, glx){
	if(!call)call='';
	if(!can)can={};
	js.uploadrand	= js.now('YmdHis')+parseInt(Math.random()*999999);
	var url = 'index.php?m=rgfee&d=public&callback='+call+'&upkey='+js.uploadrand+'';
	for(var a in can)url+='&'+a+'='+can[a]+'';
	if(glx=='url')return url;
	var s='',tit=can.title;if(!tit)tit='人工费清单制作';

	var H	= (document.body.clientHeight<578)?'500px':'578px';
	js.tanbody('rgfeewin',tit,450,300,{
...
js.clupdate=function(call,can, glx){
	if(!call)call='';
	if(!can)can={};
	js.uploadrand	= js.now('YmdHis')+parseInt(Math.random()*999999);
	var url = 'index.php?m=clupdate&d=public&callback='+call+'&upkey='+js.uploadrand+'';
	for(var a in can)url+='&'+a+'='+can[a]+'';
	if(glx=='url')return url;
	var s='',tit=can.title;if(!tit)tit='主材升级变更单';
...
//下载文件
js.downxia=function(id){
	js.open('?id='+id+'&a=down',600,350);
	return false;
}

//预览文件
js.downshow=function(id, ext,pts){
...
//生成Excel文件导出
js.createtable=function(id,field){
	js.open('?id='+id+'&a=create&field='+field+'',600,350);
	return false;
}
//生成html预览文件
js.createhtml=function(id, field){
	var url = '?id='+id+'&a=createhtml&field='+field+'';	
	if(ismobile==1){
		//if(appobj1('openfile', id))return;
		js.location(url);
	}else{
...
js.rgfeeshow=function(a, showid, totalprice,alltotal){
	var s = '',i=0,s1='';
	var o = $('#view_'+showid+'');
	var k=1;
	for(i=0; i<a.length; i++){
		if (a[i].goods) {
			s='<div onmouseover="this.style.backgroundColor=\'#f1f1f1\'" onmouseout="this.style.backgroundColor=\'\'" style="padding:4px 5px;border-bottom:1px #eeeeee solid"><span>'+k+'</span><font style="display:none">'+a[i].id+'</font>、'+a[i].goods+' 预算:'+a[i].yusuan+' '+a[i].unit+' 实际:'+a[i].shiji+' 价格:'+a[i].price+'';
			s+='</div>';
			o.append(s);
			k++;
		}
	}
		s='<div style="padding:4px 15px;border-bottom:1px #eeeeee solid"> 合计:'+alltotal+' 商定人工费总价:'+totalprice+'';
		s+='</div>';
		o.append(s);
	$('#rgfeeaddbtn').hide();
	$('#rgfeelist').val(JSON.stringify(a));
	$('#totalprice').val(totalprice);
	$('#alltotal').val(alltotal);
}
js.clupdateshow=function(a, showid, totalprice,alltotal){
	var s = '',i=0,s1='';
	var o = $('#view_'+showid+'');
	var k=1;
	...
	for(i=0; i<a.length; i++){
		if (a[i].goods) {
			s='<div onmouseover="this.style.backgroundColor=\'#f1f1f1\'" onmouseout="this.style.backgroundColor=\'\'" style="padding:4px 5px;border-bottom:1px #eeeeee solid"><span>'+k+'</span><font style="display:none">'+a[i].id+'</font>、'+a[i].goods+' 单价:'+a[i].xuanyangprice+' 数量:'+a[i].shijinum+' 变更:'+a[i].cha+'';
			//s+=' <a class="a" temp="dela" onclick="return js.downupdels('+a[i].id+',\''+showid+'\', this)" href="javascript:">×</a>';
			s+='</div>';
			o.append(s);
			k++;
	...
	$('#clupdateaddbtn').hide();
	$('#clupdatelist').val(JSON.stringify(a));
	$('#totalprice').val(totalprice);
	$('#alltotal').val(alltotal);
	//js.downupdel(0, showid, false);
}

js.paifa=function(cid,fid,action="clpaifa"){
	var url = 'index.php?m='+action+'&d=public&cid='+cid+'&fid='+fid+'';
	//var url = 'index.php?m=kaoqin&d=main&a=location&id='+cid+'';
	/*if(ismobile==1){js.location(url);return;}
...
js.tuihuo=function(cid,fid,action="clpaifa"){
	var url = 'index.php?a=tuihuo&m='+action+'&d=public&cid='+cid+'&fid='+fid+'';
	js.location(url);
	return false;
}
js.history=function(cid,fid,action="clpaifa"){
	var url = 'index.php?a=history&m='+action+'&d=public&cid='+cid+'&fid='+fid+'';
	js.location(url);
	return false;
}
