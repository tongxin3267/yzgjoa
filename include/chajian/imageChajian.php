...
class imageChajian extends Chajian
{
	...
	*/
	public function createimgobj($path)
	{
		...
		list($lw, $lh) 	= getimagesize($path);
		//手机上传的图片压缩
		//var_dump($lw,$ext);die();
		//if (($ext=='jpg'&&($lw==3120||$lw==2976||$lw==3456))||($ext=='jpeg'&&$lw==4032)) {
		if (($ext=='jpg'&&$lw>=2976)||($ext=='jpeg'&&$lw==4032)) {
			$per=0.8;
			$n_w=$lw*$per;
			$n_h=$lh*$per;
			...
			//copy部分图像并调整
			imagecopyresized($new, $img1,0, 0,0, 0,$n_w, $n_h, $lw, $lh);
			//图像输出新图片、另存为
			imagejpeg($new,$path);
			imagedestroy($new);
			imagedestroy($img1);
