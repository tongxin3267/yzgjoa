...
class inputChajian extends Chajian
{
	...
	public function getfieldcont($fid, $objs=null, $leox='', $iszb=0,$oldrs)
	{
		...
		$str 	= '<input class="inputs" type="text" value="'.$val.'" '.$attr.' name="'.$fname.'">';
		...
		if($type=='selectcity'){
			$str ='<select style="width:42%" '.$attr.' name="'.$fname.'" class="inputs">';
			$str.='<option value="">-请选择-</option>';
			$str1= '';
			$datanum = $data;
			if(!isempt($datanum)){
				...
				if($objs!=null && method_exists($objs, $datanum)){
					$fopt	= $objs->$datanum($fid,$this->mid);
					foreach($fopt as $k=>$rs){
						$sel = ($rs['name']==$val)?'selected':'';
						$str.='<option value="'.$rs['value'].'" '.$sel.'>'.$rs['name'].'</option>';
						$str1.='<label><input name="'.$fname.'[]" value="'.$rs['name'].'" type="checkbox">'.$rs['name'].'</label>&nbsp;&nbsp;';
					}
					$fopt = true;
				}
			}
			$str.='</select>';
			$str.='<input name="routelinetext" type="hidden">';

			$str.='<select style="width:42%" '.$attr.' name="'.$fname.'e" class="inputs" id="select2">';
			if(isset($oldrs)&&!empty($oldrs['routeline'])){
				$str.='<option value="'.$oldrs['routeline'].'" selected>'.$oldrs['routeline'].'</option>';
			}else{
				$str.='<option value="">-请选择-</option>';
			}
			$str.='</select>';
		...
		if($type=='uploadfile'){
			...
			$str.= '<div style="display:none" tsye="file" tnam="'.$fname.'" id="filed_'.$fname.'"><input type="file" id="filed_'.$fname.'_inp"></div>';
		}
		if($type=='auto'){
			$datanum = $data;
			if(!isempt($datanum)){
